...
function ShortagePanel({
  ...
  const shortageCol = findWeightColumn(weights, ['shortage']);

  const getBaseWeightForMonth = (month) => {
    if (!month) return 1.0;
    const monthIdx = MONTH_TO_IDX[month];
    return getBaseMultiplier(weights, shortageCol, monthIdx);
  };
  ...
  const handleLockEvent = () => {
    ...
    addLockedEvent(EVENT_TYPES.SHORTAGE, {
      ...
      {settings.month && (
        <>
          ...
          <p className="text-xs text-surface-500">
            Negative: reduce impact | Positive: increase impact (capped at 0%)
          </p>
          ...
          <Button
            ...
            className="w-full"
          >
            Lock in {settings.month} Shortage
          </Button>
        </>
      )}
          ...
          <div className="flex flex-wrap gap-1.5">
            {lockedEvents.map((event) => (
              <LockedEventItem
                key={event.month}
                event={event}
                onRemove={() => removeLockedEvent(EVENT_TYPES.SHORTAGE, event.month)}
              />
            ))}
          </div>
        </div>
