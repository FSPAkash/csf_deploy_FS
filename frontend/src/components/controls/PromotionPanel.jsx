...
import { Button, Select, Slider, Checkbox, TooltipPortal } from '../common';
...
function MultiplierMathTooltipContent({ baseWeight, boostPct, finalMultiplier, month, isCapped }) {
  const sliderMultiplier = 1 + (boostPct || 0) / 100;
  ...
  return (
    <div className="glass-strong rounded-lg p-3 shadow-xl border border-surface-200/50 whitespace-nowrap bg-white/95">
      ...
      <div className="space-y-1.5 text-xs">
        ...
        <div className="flex items-center justify-between gap-4">
          <span className="text-surface-500">Boost effect (+{boostPct}%)</span>
...
function LockedEventItem({ event, onRemove }) {
  ...
  const isCapped = event.month === 'Jun' && event.multiplier >= 1.06;
...
function PromotionPanel({
  ...
  const getPromoColumn = (month) => {
    if (!month) return null;
    const monthIdx = MONTH_TO_IDX[month];
    if (monthIdx <= 6) {
      return findWeightColumn(weights, ['upromoup']);
    } else {
      return findWeightColumn(weights, ['dpromoup']);
    }
  };
  ...
  const getBaseWeightForMonth = (month) => {
    if (!month) return 1.0;
    const monthIdx = MONTH_TO_IDX[month];
    const promoCol = getPromoColumn(month);
    return getBaseMultiplier(weights, promoCol, monthIdx);
  };
  ...
  const calculateFinalMultiplier = (month, pct) => {
    ...
    let multiplier = baseWeight * sliderAdjustment;

    if (month === 'Jun') {
      multiplier = Math.min(multiplier, 1.06);
    }
  ...
  const isCapped = settings.month === 'Jun' && previewMultiplier >= 1.06;
  ...
  return (
    <div className="space-y-4 relative">
      ...
      <div data-tutorial="promotion-month" className="relative">
      ...
      {settings.month && (
        <>
          <div data-tutorial="promotion-impact">
            <Slider
              label="Boost Effect"
          ...
          <div className="relative text-xs text-center glass-subtle p-2.5 rounded-lg border border-daikin-blue/20 bg-daikin-light/10">
            ...
            {isCapped && (
              <div className="text-amber-600 mt-1">Capped at 1.06 for June</div>
            )}
          </div>

          <div data-tutorial="lock-promotion">
            <Button
              variant="primary"
              size="sm"
      ...
      {lockedEvents.length > 0 && (
        <div className="pt-3 border-t border-surface-200/50 relative" data-tutorial="locked-events">
          ...
          <div className="flex flex-wrap gap-1.5">
            {lockedEvents.map((event) => (
              <LockedEventItem
                key={event.month}
                event={event}
                onRemove={() => removeLockedEvent(EVENT_TYPES.PROMO, event.month)}
              />
            ))}
          </div>
        </div>
